...
- `ord` CLI with a `build` subcommand. Reads `[tool.ord]` (with a nested `[tool.ord.api_resource]` table for the spec-required APIResource fields) from `pyproject.toml`, imports the `module:attr` FastAPI app, validates the resulting ORD document and Configuration manifest against the vendored JSON Schemas, and writes `gen/ord/ord-document.json` plus `gen/ord/well-known.json` (override with `--out`). Wired up via `[project.scripts]` so `pip install` puts `ord` on PATH. `examples/fastapi_app/` ships a `pyproject.toml` showing the config layout. ([#10](https://github.com/Muhazerin/ord-python/pull/10))
- ORD discovery endpoints. New `ord.server.ord_router(document_provider, *, well_known_path, document_path, base_url)` returns a FastAPI `APIRouter` mounting `/.well-known/open-resource-discovery` (a Configuration manifest pointing at the document) and `/ord/v1/documents/ord-document` (the document itself). `document_provider` accepts either an `ORDDocument` (resolved once) or a callable returning one (resolved per request). `examples/fastapi_app/` now mounts the router so `uvicorn main:app` exposes the live endpoints. ([#8](https://github.com/Muhazerin/ord-python/pull/8))
- `ORDConfiguration` and `V1DocumentDescription` Pydantic models for the manifest served at the well-known endpoint, plus `ord.core.validate_ord_configuration(data)` (and a `validate_against_spec()` method on `ORDConfiguration`) that validate against the vendored Configuration JSON Schema. ([#8](https://github.com/Muhazerin/ord-python/pull/8))
